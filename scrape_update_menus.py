...
Run
---
  python scrape_update_menus.py --shops database/csd.csv --db database/coffeeshops.sqlite --out-dir menus_downloaded
"""
...
def upsert_shop(
    ...
    closed_int = 1 if is_closed else 0
    conn.execute(
        """
        ...
        """,
        (name.strip(), city.strip(), shop_url.strip(), show_int, closed_int, now, now),
    )
    row = conn.execute(
...
def record_menu_history(
    ...
    """Append one timestamped menu observation and return its id."""
    now = utc_now_iso()
    cur = conn.execute(
        """
        INSERT INTO menu_history(
            shop_id, fetched_at_utc, source_page_url, image_url, local_path,
            sha256, bytes, event_type, status, error, created_at
        )
        VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
        """,
        (
...
def archive_active_offerings_for_rebuild(
    conn: sqlite3.Connection,
    shop_id: int,
    menu_history_id: Optional[int],
) -> int:
    """Move old active offerings out of live results when a fresh menu arrives."""
    now = utc_now_iso()
    rows = conn.execute(
        """
    ...
    for r in rows:
        conn.execute(
            ...
            VALUES(?, ?, ?, ?, 'superseded_by_new_menu', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'scraper', ?);
            """,
            (
                int(r["shop_id"]),
                int(r["strain_id"]),
    ...
    conn.execute(
        ...
        SET status = 'discontinued',
            discontinued_reason = 'new menu pending rebuild',
...
def read_shops_csv(path: str) -> List[ShopRow]:
    ...
    Supported headers:
      ...
      - optional closure flag: `is_closed` (or `closed`)
    ...
    with open(path, "r", encoding="utf-8-sig", newline="") as f:
        ...
        for r in reader:
            ...
            closed_raw = (r.get("is_closed") or r.get("closed") or "").strip()
            is_closed = parse_csv_bool(closed_raw, default=False)
            out.append(
                ShopRow(
                    shop=shop,
...
def main() -> int:
    ...
    for i, s in enumerate(shops, start=1):
        ...
        shop_id = upsert_shop(conn, s.shop, s.city, shop_url, s.show_in_admin, s.is_closed)
        if s.is_closed:
            skipped += 1
            print("  [SKIP] shop marked closed in CSV.")
            continue
        ...
        try:
            ...
            if had_known_previous_sha:
                archived_count = archive_active_offerings_for_rebuild(conn, shop_id, menu_history_id)
